...
const getProjectRoot = () => {
  ...
  try {
    ...
    console.warn(
      `Warning: schemas directory not found at ${schemasPath} or ${path.join(alternativePath, 'schemas')}`
    );
...
export async function loadSchema(schemaName: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Invalid JSON in schema file: ${schemaName} - ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
...
export async function viewSchema(schemaName: string): Promise<string> {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Error reading schema file: ${schemaName} - ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
...
export async function loadStructuredSchema(schemaName: string): Promise<ExtractorSchema> {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Invalid schema file: ${schemaName} - ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
 ...
 */
async function validateParsedSchema(
  schema: any
): Promise<{ valid: boolean; errors: any[] | null }> {
  // Validate against meta-schema
  const validate = ajv.compile(schemaMetaSchema);
...
function isStructuredSchema(schema: any): boolean {
  ...
  return schema && (schema.type || schema.selectors || schema.structure || schema.properties);
}
    ...
    if (schema.properties) {
      ...
      for (const [field, propSchema] of Object.entries(schema.properties)) {
        if (
          ...
          'selector' in propSchema &&
          typeof propSchema.selector === 'string'
        ) {
          ...
          const { selector, ...structureProp } = propSchema as {
            selector: string;
            [key: string]: any;
          };
  ...
  if (schema.fields) {
    ...
    for (const [field, selectorValue] of Object.entries(schema.fields)) {
      ...
      // Handle object selectors with proper type checking
        ...
        'selector' in selectorValue &&
        typeof selectorValue.selector === 'string'
      ) {
