...
export async function fetchHTML(
  url: string,
  options?: { retry?: number; verbose?: boolean; rateLimit?: boolean; checkRobots?: boolean }
): Promise<string> {
  ...
  if (checkRobots) {
    ...
    if (robotsCheck.crawlDelay && useRateLimit) {
      ...
      if (verbose && !stats.canMakeRequest) {
        console.log(
          `Rate limit: waiting ${Math.ceil(stats.nextAvailableIn / 1000)}s before request`
        );
      }
  ...
  throw new Error(
    `Failed to fetch HTML from ${url} after ${retries + 1} attempts: ${lastError?.message}`
  );
}
